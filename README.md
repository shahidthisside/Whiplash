...
- On Vivo/iQOO OriginOS, the lockscreen media widget can show a blank elapsed/remaining time and an unmoving progress bar for a track that's already playing, correcting itself only after a discrete player action (pause/play, skip). Whiplash's `MediaSession` already sends periodic position updates by default (Media3's standard behavior, confirmed active and unmodified in this project); the widget itself appears not to repaint on every update while the screen is off, only on outright state transitions. The same category of bug is independently reported, and left unresolved as OEM-specific, in other open-source Media3-based players (e.g. Auxio's [issue #20](https://github.com/OxygenCobalt/Auxio/issues/20), tagged `device-specific`), so this is treated as a vendor lockscreen rendering quirk rather than a gap in the app's own session implementation.
